...
function loadDependencies(projectPath = '.') {
    ...
    const content = JSON.parse(readFileSync(depsPath, 'utf-8'));
...
function generatePackageJson(resolvedFeatures, existingPkg, _projectPath = '.') {
    ...
    const ordered = { $schema: 'https://json.schemastore.org/package' };
...
function setupCueMod() {
    ...
    writeFileSync('cue.mod/module.cue', `module: "project.local"
language: {
\tversion: "v0.15.1"
}
`);
