...
function validateManifest(args: string[]): void {
  if (!checkCue()) process.exit(1);
...
function loadEcosystemManifest(): EcosystemManifest | null {
  ...
  for (const loc of locations) {
    if (existsSync(loc)) {
      try {
        return JSON.parse(readFileSync(loc, 'utf-8')) as EcosystemManifest;
      } catch {
        continue;
      }
    }
...
function validateStructure(args: string[]): void {
  const parsed = parseArgs(args, ['path']);
